...
- Native Kanban tools: `kanban_show`, `kanban_comment`, `kanban_pass_review`, `kanban_complete`, `kanban_request_changes`, and `kanban_block`.
...
|---|---|---|
| Approve | Acceptance criteria and verification pass | `kanban_pass_review` when an intent validator/original author remains; `kanban_complete` only for a legacy card with no remaining lifecycle owner |
...
Approve only when the acceptance criteria are satisfied and the evidence is sufficient. When `kanban_show` records an intent validator or original author who still owns acceptance, call:

```text
kanban_pass_review(
    summary="Reviewed and approved. <what was verified>",
    evidence={"review_outcome": "approved", "reviewer_checks": [...]}
)
```

This routes the same card to intent review; technical QA is not live acceptance. Include the exact checks that passed and any bounded caveat that does not block acceptance. Use `kanban_complete` only as a legacy fallback when the card has no lifecycle metadata and no intent validator or original author remains.
