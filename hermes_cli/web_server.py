...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        status["gateway_count"] = topology.get(
            "gateway_count",
            len(topology.get("gateways") or []),
        )
...
def _trigger_cron_job_sync(job_id: str, profile: Optional[str] = None):
    ...
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    _assert_untrusted_cron_job_allowed_for_profile(selected, job["id"])
...
def _assert_untrusted_cron_job_allowed_for_profile(
    profile: str,
    job_id: str,
) -> None:
    """Authorize a dashboard fire in the selected profile before its CAS claim."""
