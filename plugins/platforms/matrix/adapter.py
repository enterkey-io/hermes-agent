...
def _secure_matrix_traversal_supported() -> bool:
...
def _reject_windows_matrix_state_symlinks(path: Path) -> str:
    ...
    for candidate in reversed(candidates):
        if os.path.islink(candidate):
            raise _PrivateStatePermissionError(
                f"Matrix private state path contains a symlink: {path}"
            )
    return absolute


def _chmod_windows_matrix_state_path(
    path: Path,
    mode: int,
    *,
    directory: bool,
    ...
    """Preserve Matrix store initialization on Windows without dir_fd."""
    try:
        absolute = _reject_windows_matrix_state_symlinks(path)
...
def _open_matrix_state_path(
    ...
    *,
    ...
    nofollow = getattr(os, "O_NOFOLLOW", 0)
    directory_flag = getattr(os, "O_DIRECTORY", 0)
    if not _secure_matrix_traversal_supported():
        raise _PrivateStatePermissionError(
...
def _chmod_matrix_state_path(
    ...
    *,
    ...
    if os.name == "nt":
        _chmod_windows_matrix_state_path(
            path,
            mode,
            directory=directory,
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def __init__(
        ...
        *,
        ...
        homeserver_host = urlsplit(self._homeserver).hostname
        self._proxy_url: str | None = resolve_proxy_url(
            platform_env_var="MATRIX_PROXY",
            target_hosts=homeserver_host,
        )
        if self._proxy_url:
        ...
        if content:
            ...
            if prompt and not prompt.resolved:
                ...
                try:
                    ...
                    count = resolve_gateway_approval(prompt.session_key, choice, request_id=prompt.request_id) if prompt.request_id else 0
                    if count:
