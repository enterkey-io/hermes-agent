...
def run_codex_stream(agent, api_kwargs: dict, client: Any = None, on_first_delta: callable = None):
    ...
    for attempt in range(max_stream_retries + 1):
        ...
        try:
            ...
            err_text = str(exc)
            sdk_null_output_parse = "'NoneType' object is not iterable" in err_text
            if sdk_null_output_parse and attempt < max_stream_retries:
                logger.debug(
                    "Responses stream SDK parser saw response.output=None "
