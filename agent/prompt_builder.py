...
KANBAN_GUIDANCE = (
    ...
    "task), unless the current dispatcher query begins with the exact "
    "`[HERMES_HOST_TERMINAL_REVIEW_V1]` marker. That marker identifies a "
    "host-generated, task/run-bound terminal-review snapshot and replaces only "
    "the redundant opening discovery call; use `kanban_show()` if its identities "
    "or state later conflict with tool results. Ordinary workers, including a "
    "query beginning with `[HERMES_HOST_COORDINATED_WORK_V1]`, never skip the "
    "opening call. That work marker carries a host-observed launch-time budget; "
    "read the current `coordination_budget` returned by `kanban_show()`. Its "
    "`phase_model_calls_remaining` is the usable work balance, while the "
    "aggregate balance includes a terminal reserve unavailable to the work "
    "phase. Admission rechecks before every provider call, so plan to reserve "
    "the final work-phase response for one truthful lifecycle handoff. The "
    "response also includes title, body, parent-task handoffs "
    "(summary + metadata), any prior attempts on this task if you're a retry, "
    "the full comment thread, and a pre-formatted `worker_context` you can treat "
    "as ground truth for board state. Implementer summaries and comments remain "
    ...
    "not trip unblock-loop detection. When the task explicitly requires a "
    "partial handoff parked until a manager release, record the evidence and "
    "call `kanban_block(kind=\"scheduled\", reason=...)`. This host-side "
    "handoff works even with a remote terminal backend. A successful schedule "
    "ends this worker run without completing the overall task. Stop then; "
    "do not unblock yourself or start another phase.\n"
