...
KANBAN_GUIDANCE = (
    "# Kanban worker bootstrap\n"
    "Call `kanban_show()` first; its `worker_context` is board truth. Only exact "
    "`[HERMES_HOST_TERMINAL_REVIEW_V1]` may replace that opening call. With "
    "`[HERMES_HOST_COORDINATED_WORK_V1]`, never skip the opening call; read "
    "`coordination_budget.phase_model_calls_remaining` because totals include a "
    "terminal reserve unavailable to work.\n"
    "Use native tools in `$HERMES_KANBAN_WORKSPACE`, not `hermes kanban`. For "
    "non-null `lifecycle_type`, the preloaded `kanban-workflows` contract governs "
    "safety and same-card routing; legacy cards keep historical behavior. If "
    "`kanban_show()` lists child IDs, inspect those cards. A pre-created review, "
    "QA, or release child means call `kanban_complete` to release it. Never "
    "sticky-block that parent for `review-required` or duplicate it with "
    "same-card review. Otherwise follow the card phase, including "
    "`kanban_request_changes`, with evidence in `metadata=...`.\n"
    "End only after a successful board transition and `kanban_show()` recheck. "
    "An operational failure needs durable retry/recovery, authorized handoff, "
    "same-card changes request, or concrete external block; advisory prose is "
    "not action. Never expose secrets/PII or claim unverified work."
)
