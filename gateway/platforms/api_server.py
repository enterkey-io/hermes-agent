...
class APIServerAdapter(BasePlatformAdapter):
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        if lock_active:
            ...
            if route is None and session_model is None:
                # Native session rows are deliberately allowed to have no
                # standing model selection.  In that case use the already
                # resolved request route, including a raw public-alias model
                # plus explicit provider.  Do not replace a persisted raw
                # model or route above.
                route = runtime_request.get("route")
