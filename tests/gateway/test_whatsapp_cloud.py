...
class TestSendExecApprovalButtons:
    ...
    async def test_approval_renders_two_buttons(self):
        ...
        assert adapter._exec_approval_state[approval_id] == {"session_key": "sess-app-1", "request_id": None}
...
class TestDispatchInteractiveReplyApproval:
    ...
    async def test_button_is_bound_and_duplicate_is_consumed(self, monkeypatch, binding):
        from tests.gateway._approval_binding import pending_pair

        adapter = _make_adapter()
        adapter._http_client = MagicMock()
        adapter._http_client.post = AsyncMock(
            return_value=_mock_httpx_response(200, {"messages": [{"id": "wamid.binding"}]})
        )
        ...
        await adapter.send_exec_approval("15551234567", "fixture", "sess-binding", request_id=request_id)
        payload = adapter._http_client.post.call_args.kwargs["json"]
        button = payload["interactive"]["action"]["buttons"][0]["reply"]
        ...
        # A replay must not fall through to its plain-text "Approve" title.
        assert await adapter._dispatch_interactive_reply(raw, {}) is True
        assert older.result is None
    ...
    async def test_approve_tap_calls_resolver_and_confirms(self, monkeypatch):
        adapter = _make_adapter()
        adapter._exec_approval_state["app1"] = {"session_key": "sess-app-1", "request_id": "req-1"}
        adapter._http_client = MagicMock()
        adapter._http_client.post = AsyncMock(
            return_value=_mock_httpx_response(200, {"messages": [{"id": "x"}]})
        )
        ...
        monkeypatch.setattr(
            ...
            lambda session_key, choice, request_id: calls.append((session_key, choice, request_id)) or 1,
        )
        ...
        assert handled is True
        assert calls == [("sess-app-1", "once", "req-1")]
