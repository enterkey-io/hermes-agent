...
def _job(**overrides):
    job = {
        ...
        "prompt": "hello",
        # Disabled legacy jobs retain their unpinned snapshot contract on save;
        # newly enabled jobs are now fully pinned by the upstream store layer.
        "enabled": False,
        "state": "paused",
...
def _tick(job, tmp_path, current_provider, deliveries):
    ...
    with patch("cron.scheduler._hermes_home", tmp_path), \
         patch("cron.scheduler.load_config", return_value={"cron": {"model_drift_guard": True}}), \
