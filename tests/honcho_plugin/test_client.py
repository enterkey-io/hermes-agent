...
class TestResolveActiveHost:
    ...
    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"HERMES_HONCHO_HOST": "hermes.coder"}):
            with pytest.raises(ValueError, match="not allowed for profile"):
                resolve_active_host()
...
class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self, tmp_path):
        ...
        context = HonchoProfileContext.for_profile(
            "coder", root, default_root=tmp_path
        )
        config = HonchoClientConfig.from_env(
            host="hermes_coder", context=context
        )
...
class TestGetHonchoClient:
    ...
    def test_dot_form_legacy_host_key_keeps_local_api_key(self, tmp_path):
        ...
        root.mkdir(parents=True)
        context = HonchoProfileContext.for_profile(
            "work", root, default_root=tmp_path
        )
...
class TestGetHonchoClientBaseUrlDoublePrefixFix:
    ...
    def test_lan_default_host_empty_key_uses_local_placeholder(self, tmp_path):
        """Regression for #61661: setup-style root baseUrl + canonical host + LAN IP
